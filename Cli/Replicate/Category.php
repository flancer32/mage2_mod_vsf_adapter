...
use Flancer32\VsfAdapter\Service\Replicate\Category\Full\Request as AFullRequest;
    ...
    extends \Symfony\Component\Console\Command\Command
{
    ...
    private $srvReplCatFull;

    public function __construct(
        \Magento\Framework\ObjectManagerInterface $manObj,
        \Magento\Framework\App\State $appState,
        \Flancer32\VsfAdapter\Service\Replicate\Category\Full $srvReplCatFull
    ) {
        /* these objects are used in parent::__construct/configure */
        $this->manObj = $manObj;
        $this->appState = $appState;
        /* these objects are own props */
        $this->srvReplCatFull = $srvReplCatFull;
    ...
    protected function execute(
        ...
        $req = new AFullRequest();
        $req->indexPrefix = $prefix;
        $req->storeId = $storeId;
        $this->srvReplCatFull->execute($req);
    ...
    private function initOptions()
    {
        $this->addOption(
            ...
            'Prefix to construct names for ElasticSearch index (`vsf_msk` => `vsf_msk_category`).'
        );
        $this->addOption(
            self::OPT_STORE,
